    ...
    <section className="relations-stage relations-stage--review">
      <section className="relations-summary">
        <div className="relations-summary__head">
          <div>
            <p className="dashboard-hero__eyebrow">Review</p>
          ...
          <div className="relations-summary__actions">
            ...
            <button type="button" onClick={onDownloadFavoritePack}>
              הורד סיכום
            </button>
        ...
        <div className="relations-review-grid">
          ...
          <section className="panel-card">
            <h3>המצב הרגשי עכשיו</h3>
            {latestTurn ? (
              <div className="relations-review-card">
                <strong>{latestTurn.emotionAfter?.labelHe ?? 'ללא שינוי רגשי מובהק'}</strong>
                <p>{latestTurn.emotionAfter?.intensity ? `עוצמה ${latestTurn.emotionAfter.intensity}/5` : 'הרגש עוד לא עודכן.'}</p>
                <p>{latestTurn.relationShift?.relationLabelBefore} ← → {latestTurn.relationShift?.relationLabelAfter}</p>
              </div>
            ) : (
        ...
        <div className="relations-summary-grid">
          ...
          <div className="relations-panel">
            ...
            {likedTurns.length === 0 && session.turns.length > 0 && (
